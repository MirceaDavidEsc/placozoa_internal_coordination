...
simDir = ifelse(fluidSimulation, "SimData_Szabo_new_out", "SimData")

simDir = paste(projectFolder, "/Data_Original/PawelSimulations/",simDir,"/", sep = "")
simSuffix = ifelse(fluidSimulation, "_fluid", "_fixed")
...
criticalNoise = ifelse(fluidSimulation, 1.45, 1.1)

criticalityFiles = filter(metadata, Size == 16384 & Noise == criticalNoise)
criticalityFrames = read_rds(criticalityFiles$filename[[1]])
selectedFrame = unique(criticalityFrames$Frame) # Arbitrarily selected frame
...
write_rds(x = criticalityFrame, path = paste(projectFolder,"/Data_Processed/criticalSnapshot", simSuffix,".rds", sep = ""))
...
if (fluidSimulation) {
  criticalRange = c(1.4, 1.8)
} else {
  criticalRange = c(1.3, 1.6)
}
...
(orderNoisePlot = ggplot(orderDataSummary, aes(Noise, meanOrder, color = sqrt(Size), group = Size)) + geom_point() + geom_line() + scale_y_continuous("Order") +
  ...
  scale_color_gradient(TeX("\\sqrt{N}")) + annotate("rect", ymax = Inf, ymin = -Inf, xmin = criticalRange[[1]], xmax = criticalRange[[2]], fill = "red", alpha = 0.5))


save_plot(paste(projectFolder,"/figures/3B_NoiseOrder",simSuffix,".pdf", sep = ""), orderNoisePlot)

write_csv(orderDataSummary, paste(simDir, "/orderData", simSuffix, "csv", sep = ""))
```
...
energyataFile = ifelse(fluidSimulation, "energyData_fluid.rds", "energyData_fixed.rds")

write_rds(energyMeasures, paste(simDir, energyataFile, sep = "/"))
```



```{r fluctuationEnergyAnimal}
energyOrder = inner_join(read_csv(paste(simDir, orderDataFile, sep = "")), 
                         read_rds(paste(simDir, energyataFile, sep = "")))
...
(noiseenergyRelationPlot = ggplot(energyNoiseRelated, aes(Noise, meanEnergy, color = sqrt(Size), group = Size)) + 
...
criticalRange = energyOrder %>% filter(Noise >= criticalRange[[1]] & Noise <= criticalRange[[2]])

(criticalSlopesPlot = ggplot(criticalRange, aes(energyRatio, meanOrder, group = Size)) + 
