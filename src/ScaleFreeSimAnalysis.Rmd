...
params:
  sim_dir: "D:/Mircea/Projects/RESEARCH/InternalCoordination/PawelSimulations/SimData_nonnorm_fluid/"
  timeLag: 3
---
...
# My own packages. Call the below function if they do not load
#package_installs()
...
cl = multidplyr::new_cluster(11)
cl = cluster_library(cl, packages = c("dplyr", "purrr", "tidyr", "stringr", "readr", "collective", "bigsplines", "alphahull"))
#TODO: need to fix "scalefree" to add it back into cluster library
...
simDir = paste(projectFolder, "/Data_Original/PawelSimulations/SimData_nonnorm_fluid/", sep = "")
...
metadata = list_sim_data_info(simDir, "*sampledFrames.rds")

criticalityFiles = filter(metadata, Size == 16384 & Noise == 0.35)
...
write_rds(x = criticalityFrame, path = paste(projectFolder,"FigureData/simulation_sample_frame_noise_fluid.rds", sep = ""))
save_plot(paste(paste(projectFolder,"FigureData/", sep = ""), "3A_criticalNoise.pdf", sep = "/"), criticalNoisePlot)
...
(orderNoisePlot = ggplot(orderDataSummary, aes(Noise, meanOrder, color = sqrt(Size), group = Size)) + geom_point() + geom_line() + scale_y_continuous("Order") + scale_x_continuous(expression(eta), breaks = seq(0,0.8,0.1)) + 
  scale_color_gradient(TeX("\\sqrt{N}")) + annotate("rect", ymax = Inf, ymin = -Inf, xmin = 0.32, xmax = 0.37, fill = "red", alpha = 0.5))
save_plot(paste(paste(projectFolder,"FigureData/", sep = ""), "3B_NoiseOrder_fluid.pdf", sep = "/"), criticalNoisePlot)

write_rds(orderDataSummary, paste(projectFolder, "orderData_noise.rds", sep = ""))
...
write_rds(energyMeasures, paste(projectFolder, "FigureData/energyData_noise_fluid.rds", sep = "/"))
```
...
(noiseEnergyCriticalityPlot = filter(energyNoiseRelated, Noise >= 0.31 & Noise <= 0.375) %>% ggplot(aes(Noise, meanEnergy, color = Size, group = Size)) + geom_point() + geom_line() + scale_x_continuous(expression(eta), breaks = c(0.33, 0.35, 0.37)) + scale_y_continuous(expression(paste(eta,"*",sep=""))) + scale_color_continuous(TeX("\\sqrt{N}")))
noiseEffectiveRelation = plot_grid(noiseenergyRelationPlot, noiseEnergyCriticalityPlot, rel_widths = c(0.4,0.6))
save_plot(paste(simDir, "/noiseEffectiveRelation.pdf", sep = ""), noiseEffectiveRelation, base_height = 6, base_width = 12)


criticalRange = energyOrder %>% filter(Noise >= 0.3 & Noise <= 0.4) %>% filter(abs(rotation) <= 0.3)
write_rds(x = criticalRange, path = paste("D:/Mircea/Projects/RESEARCH/InternalCoordination/FigureData/3C_NoiseOrderRelation_noise_fluid.rds"))
...
write_rds(modelSlopes, "D:/Mircea/Projects/RESEARCH/InternalCoordination/FigureData/3D_NoiseSlope_noise_fluid.rds")
...
write_rds(correlationProfiles, "D:/Mircea/Projects/RESEARCH/InternalCoordination/FigureData/correlationProfiles_sim_noise_fluid.rds")
```
...
write_rds(perFrameAvgCorrStats, "D:/Mircea/Projects/RESEARCH/InternalCoordination/FigureData/correlationStats_sim_noise_fluid.rds")
...
write_rds(perFrameAvgCorrStats, "D:/Mircea/Projects/RESEARCH/InternalCoordination/FigureData/3E_3F_corrStats_noise_fluid.rds")
...
write_rds(x = orderSuscept, path = paste(projectFolder,"simOrderSusceptibility_noise.rds", sep = "/"))
