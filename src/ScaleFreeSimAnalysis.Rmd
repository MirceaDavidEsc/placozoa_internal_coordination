...
simulationTypes = data.frame(simType = c("_fluid", "_fixed", "_fluid_fast", "_nonormspring"),
                             ...
                             criticalMin = c(1.4, 1.3, 1.4, 3.2), criticalMax = c(1.8, 1.6, 1.8, 4.0), 
...
(orderNoisePlot = ggplot(orderDataSummary, aes(Noise, meanOrder, color = sqrt(Size), group = Size)) + 
    geom_point() + geom_line() + scale_y_continuous("Order") +
...
(criticalNoisePlot = plot_simulation_snapshot(criticalityFrame, arrows = T, arrows_round = 25, arrowMultiple = 50) +
...
(simulationOrderPlot = plot_grid(top2, insetGraph, nrow = 2, labels = c("", "C")))
