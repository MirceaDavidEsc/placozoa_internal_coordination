...
params:
  sim_dir: "PawelSimulations/SimData_nonnorm"
timeLag: 3
---
...
simDir = 'D:/Mircea/Projects/RESEARCH/InternalCoordination/PawelSimulations/PawelFluid' #paste(projectFolder,params$sim_dir, sep="")
```
...
selectedDFs = filter(metadata, Size == 16384 & Noise %in% c(0.1, 0.6, 0.35) & Replicate == 0) %>% arrange(Noise)
lowNoise = read_rds(selectedDFs$filename[[1]]) %>% filter(Frame == unique(Frame)[1]) %>% select(-Frame) %>% mutate(angle = atan2(x = vX, y = vY), speed = sqrt(vX^2 + vY^2))
(lowNoisePlot = ggplot(lowNoise, aes(X, Y, color = angle, alpha = speed)) + geom_point() + coord_fixed() + scale_color_gradientn(name = expression(theta), colours = cbPalette, limits = c(-pi,pi), breaks=c(-pi, 0, pi), labels=c(expression(paste("-",pi,sep="")), 0, expression(paste(pi)))) + guides(alpha = F) + theme(axis.line = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), axis.title = element_blank()))

criticalNoise = read_rds(selectedDFs$filename[[2]]) %>% filter(Frame == unique(Frame)[1]) %>% select(-Frame) %>% mutate(angle = atan2(x = vX, y = vY), speed = sqrt(vX^2 + vY^2))
(criticalNoisePlot = ggplot(criticalNoise, aes(X, Y, color = angle, alpha = speed)) + geom_point() + coord_fixed() + scale_color_gradientn(name = expression(theta), colours = cbPalette, limits = c(-pi,pi), breaks=c(-pi, 0, pi), labels=c(expression(paste("-",pi,sep="")), 0, expression(paste(pi)))) + guides(alpha = F) + theme(axis.line = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), axis.title = element_blank()))


highNoise = readRDS(selectedDFs$filename[[3]]) %>% filter(Frame == unique(Frame)[1]) %>% select(-Frame) %>% mutate(angle = atan2(x = vX, y = vY), speed = sqrt(vX^2 + vY^2))
(highNoisePlot = ggplot(highNoise, aes(X, Y, color = angle, alpha = speed)) + geom_point() + coord_fixed() + scale_color_gradientn(name = expression(theta), colours = cbPalette, limits = c(-pi,pi), breaks=c(-pi, 0, pi), labels=c(expression(paste("-",pi,sep="")), 0, expression(paste(pi)))) + guides(alpha = F) + theme(axis.line = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), axis.title = element_blank()))

save_plot(paste(simDir, "lowNoise.pdf", sep = "/"), lowNoisePlot)
save_plot(paste(simDir, "criticalNoise.pdf", sep = "/"), criticalNoisePlot)
save_plot(paste(simDir, "highNoise.pdf", sep = "/"), highNoisePlot)
```
...
# Function: parallelized implementation for calculating order measures (1 animal per cluster)
...
metadata$orderFile = paste(dirname(metadata$processedFile), "/order",basename(metadata$processedFile), sep="")
needsOrder = filter(metadata, !file.exists(orderFile))

# If there are any folders that need order, calculate the order.
if (nrow(needsOrder) != 0) {
  cluster_copy(cl, mapOrderMeasure)
  orderData = needsOrder %>% mutate(processedFile = paste(simDir, processedFile, sep = "/")) %>% partition() %>% 
    mutate(orderDF = map(processedFile, mapOrderMeasure)) %>% collect() %>% ungroup()
  orderData = orderData %>% select(processedFile, Noise, Size, Replicate, orderDF) %>% mutate(processedFile = basename(processedFile)) %>% unnest()
}


orderData = orderData %>% arrange(Noise, Size)
write_rds(orderData, paste(simDir, "orderData.rds", sep = "/"))


orderDataSummary = group_by(orderData, Noise, Size) %>% summarise(meanOrder = mean(Order), sdOrder = sd(Order), count = n())

(orderNoisePlot = ggplot(orderDataSummary, aes(Noise, meanOrder, color = sqrt(Size), group = Size)) + geom_point() + geom_line() + geom_errorbar(aes(ymax = meanOrder + sdOrder/sqrt(count), ymin = meanOrder - sdOrder/sqrt(count)), width = 0.05) + scale_y_continuous("Order") + scale_x_continuous(expression(eta), breaks = seq(0,0.8,0.1)) + 
    scale_color_gradient(TeX("\\sqrt{N}")) + annotate("rect", ymax = Inf, ymin = -Inf, xmin = 0.32, xmax = 0.37, fill = "red", alpha = 0.5))

(orderSizePlot = ggplot(orderDataSummary, aes(sqrt(Size), meanOrder, color = Noise, group = Noise)) + geom_point() + geom_line() + scale_colour_gradient2(high = "blue", low = "black", mid = "red", midpoint = 0.35))
```
...
# If the old data existed, merge it with the newly calculated data.
if (exists("oldEnergyData")) {
  if (exists("energyMeasures")) {
    energyMeasures = rbind(energyMeasures, oldEnergyData)
  } else {
    energyMeasures = oldEnergyData
  }
}
energyMeasures = energyMeasures %>% arrange(Noise, Size)
...
criticalRange = energyOrder %>% filter(Noise >= 0.3 & Noise <= 0.4) %>% filter(abs(rotation) <= 0.3)
...
correlationStats = nestedProfiles %>% partition(Noise) %>% cluster_library("pracma") %>% cluster_copy(mapCorrStats) %>% mutate(corrStats = map(correlationProfile, mapCorrStats)) %>% 
  collect() %>% select(Noise, Size, Replicate, Frame, corrStats) %>% unnest() %>% ungroup() %>% arrange(Noise)

perFrameAvgCorrStats = group_by(correlationStats, Noise, Size) %>%
  summarise_each(funs(mean, sd), c(vZero:sSuscept)) %>% mutate(nObs = 100)

criticalData = filter(perFrameAvgCorrStats, Noise == 0.35)
...
save_plot(plot = simPlot, paste(simDir, "simPlot.pdf", sep = "/"), base_width = 14, base_height = 18)
```
...
susceptOrderPlot = ggplot(orderSuscept, aes(Noise, susceptibility, group = sqrt(Size), color = sqrt(Size))) + geom_point() + geom_line() + scale_y_continuous(expression(chi[o])) + scale_x_continuous(expression(eta)) + scale_color_gradient(TeX("\\sqrt{N}")) + ggtitle("Susceptibility from order")
save_plot(filename = paste(simDir, "susceptFromPolarization_norotation.jpg", sep = "/"), susceptOrderPlot)
...
# ## Troubleshooting plots below: check out distributions for order parameters for different sizes and noises.
# badBinder = filter(orderBinder, binderCumulant < 0.3)
# badBinder = unique(badBinder)
# dir.create(paste(simDir, "debugHistograms", sep = "/"))
# for (i in 1:nrow(badBinder)) {
#   badSize = badBinder$Size[[i]]
#   badNoise = badBinder$Noise[[i]]
#   badOrder = orderMeasures %>% filter(Size == badSize, Noise == badNoise)
#   orderHistogram = ggplot(badOrder, aes(rotation)) + geom_histogram()
#   save_plot(paste(simDir, "/debugHistograms/debug_N",badSize,"_eta",badNoise,".jpg",sep=""), orderHistogram)
# }
# 
# ggplot(orderMeasures, aes(as.factor(Noise), Order)) + geom_boxplot() + facet_wrap(~ Size)
```
